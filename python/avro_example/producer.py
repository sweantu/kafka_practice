...
class RideAvroProducer:
    def __init__(self, props: Dict):
        ...
        self.key_serializer = AvroSerializer(
            schema_registry_client, key_schema_str, ride_record_key_to_dict
        )
        self.value_serializer = AvroSerializer(
            schema_registry_client, value_schema_str, ride_record_to_dict
        )
    ...
    def read_records(resource_path: str):
        ...
        with open(resource_path, "r") as f:
            ...
            for row in reader:
                ride_records.append(
                    RideRecord(arr=[row[0], row[3], row[4], row[9], row[16]])
                )
